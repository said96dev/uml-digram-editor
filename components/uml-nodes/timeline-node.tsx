...
export const TimelineNode = memo(({ data, selected }: NodeProps<TimelineNodeData>) => {
  const [name, setName] = useState(data.name || "Timeline")

  return (
    <div className="relative bg-cyan-50 border-2 border-gray-300 shadow-md" style={{ width: 300, minHeight: 150 }}>
      <NodeResizer
        color="#1971c2"
        isVisible={selected}
        minWidth={300}
      ...
      {/* Time labels */}
      <div className="absolute top-20 left-0 text-xs text-gray-800 font-medium">t0</div>
      <div className="absolute top-20 left-1/4 text-xs text-gray-800 -translate-x-1/2 font-medium">t1</div>
      <div className="absolute top-20 left-2/4 text-xs text-gray-800 -translate-x-1/2 font-medium">t2</div>
      <div className="absolute top-20 left-3/4 text-xs text-gray-800 -translate-x-1/2 font-medium">t3</div>
      <div className="absolute top-20 right-0 text-xs text-gray-800 -translate-x-full font-medium">t4</div>
